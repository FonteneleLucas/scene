...
Shape.prototype.transformMatrix = function () {
  ...
  var R = multMatrix(
    rotateMatrixX(this.rotate.x),
    multMatrix(rotateMatrixY(this.rotate.y), rotateMatrixZ(this.rotate.z))
  ); //TODO: modificar para receber a matriz de rotação
...
Shape.prototype.transformMatrixInverse = function () {
  var Ti = translateMatrixI(
    this.translate.x,
    this.translate.y,
    this.translate.z
  ); //TODO: modificar para receber a matriz de escala
  var Ri = multMatrix(
    rotateMatrixXI(this.rotate.z),
    multMatrix(rotateMatrixYI(this.rotate.y), rotateMatrixZI(this.rotate.x))
  ); //TODO: modificar para receber a matriz de rotação
...
Shape.prototype.testCuboIntersection = function (ray) {
  var half = this.height / 2;
  var min_x = this.center.x - half;
  var min_y = this.center.y - half;
  var min_z = this.center.z - half;
  var max_x = this.center.x + half;
  var max_y = this.center.y + half;
  var max_z = this.center.z + half;

  var tx1 = (min_x - ray.origin.x) / ray.direction.x;
  var tx2 = (max_x - ray.origin.x) / ray.direction.x;
  var ty1 = (min_y - ray.origin.y) / ray.direction.y;
  var ty2 = (max_y - ray.origin.y) / ray.direction.y;
  var tz1 = (min_z - ray.origin.z) / ray.direction.z;
  var tz2 = (max_z - ray.origin.z) / ray.direction.z;

  var tmin = Math.max(
    Math.max(Math.min(tx1, tx2), Math.min(ty1, ty2)),
    Math.min(tz1, tz2)
  );
  var tmax = Math.min(
    Math.min(Math.max(tx1, tx2), Math.max(ty1, ty2)),
    Math.max(tz1, tz2)
  );
...
Shape.prototype.testCylinderIntersection = function (ray) {
  ...
  var b =
    2 *
    (Vec.dot(ray.d, Vec.minus(ray.o, center)) -
      Vec.dot(ray.d, axis) * Vec.dot(Vec.minus(ray.o, center), axis));
  var c =
    Vec.dot(Vec.minus(ray.o, center), Vec.minus(ray.o, center)) -
    Math.pow(Vec.dot(Vec.minus(ray.o, center), axis), 2) -
    Math.pow(radius, 2);
...
Shape.prototype.testIntersectionRay = function (ray) {
  ...
  if (this.geometry == sphere) {
    //testar interseção com a esfera
  ...
  } else if (this.geometry == cubo) {
        t = this.testCuboIntersection(ray);
    //     if (t !== undefined) {
    //       var point = ray.get(t);
    //       var epsilon = 0.0001;
    //       var normal = new Vec3();
    //       if (Math.abs(point.x - 0.5) < epsilon) {
    //         normal.x = 1;
    //       } else if (Math.abs(point.x + 0.5) < epsilon) {
    //         normal.x = -1;
    //       } else if (Math.abs(point.y - 0.5) < epsilon) {
    //         normal.y = 1;
    //       } else if (Math.abs(point.y + 0.5) < epsilon) {
    //         normal.y = -1;
    //       } else if (Math.abs(point.z - 0.5) < epsilon) {
    //         normal.z = 1;
    //       } else if (Math.abs(point.z + 0.5) < epsilon) {
    //         normal.z = -1;
    //       }
